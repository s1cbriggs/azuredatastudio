...
export async function checkRedirectServer(adfs: boolean) {
	...
	const promise = new Promise<boolean>(resolve => {
		// TODO: karlb rewrote, does this still work 11/19
		const req = https.get(url.parse(`${redirectUrlAAD}?state=3333,cccc`),
		res => {
...
function createTerminateServer(server: http.Server) {
	...
	return async () => {
		const result = new Promise<void>(resolve => server.close(<any>resolve));
...
function createServer(nonce: string) {
	// TODO: karlb does this still work 11/19
	...
	const server = http.createServer(function (req, res) {
		...
		switch (reqUrl.pathname) {
			case '/signin':
				const receivedNonce = (<any>(reqUrl.query.nonce || '')).replace(/ /g, '+');
